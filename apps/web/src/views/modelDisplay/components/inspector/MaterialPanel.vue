<template>
  <div class="material-panel" v-if="material">
    <h3>材质</h3>
    ...
    <div class="section">
      <h4>贴图</h4>
      <button @click="changeTexture">修改贴图</button>
    </div>
  </div>
...
const material = computed(() => {
  const object = sceneStore.selectedObject
...
const withMaterialCommand = (applyChange: (mat: THREE.MeshBasicMaterial) => void) => {
...
const changeTexture = () => {
  const sunTex = assetsStore.loadedAssets['sun']
  console.log(sunTex)
  if(sunTex && sunTex instanceof THREE.Texture) {
    withMaterialCommand((mat) => {
      mat.map = sunTex
    });
  }
